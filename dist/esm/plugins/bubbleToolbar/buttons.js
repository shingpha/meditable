 ...
 */
import { tokenizer } from '../../modules/content/inlineRenderers/tokenizer/index.js';
import { getFormatsInRange } from '../../modules/content/commands/format.js';
...
const BLACKLIST_BLOCK_TYPES = new Set([
...
function defaultIsActive(cmdName) {
    const target = COMMAND_TYPE_MAP[cmdName] ?? cmdName;
    return ({ formats }) => formats.some(f => f.type === target ||
        f.type === cmdName ||
...
function resolveItems(raw, registeredCmds) {
...
function flattenLeafBlocksBetween(root, startId, endId) {
    const out = [];
    let started = false;
    let done = false;
    (function walk(node) {
...
function formatsForBlock(block, start, end) {
    const tokens = tokenizer(block.text ?? '');
    return getFormatsInRange({ offset: start }, { offset: end }, tokens).formats;
}
function getActiveMap(items, cursor, contentData) {
    const map = {};
    ...
    try {
        if (sameBlock) {
            ...
            const blocks = flattenLeafBlocksBetween(contentData, cursor.anchorBlockId, cursor.focusBlockId);
...
function getEnabledMap(items, cursor, contentData) {
    const map = {};
...
export { BLACKLIST_BLOCK_TYPES, DEFAULT_ITEMS, defaultIsActive, getActiveMap, getEnabledMap, resolveItems };
