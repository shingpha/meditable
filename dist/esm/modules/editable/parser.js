...
function unhtml(str, reg) {
    return str ?
        str.replace(reg ||
            /[&<">'](?:(amp|lt|ldquo|rdquo|quot|gt|#39|nbsp|middot|#\d+);)?/g, function (a, b) {
            if (b) {
                return a;
            }
            else {
...
function insertText(node, arr) {
    const parentNode = node.parentNode();
    if (parentNode && parentNode.tagName() === "pre") {
        arr.push(node.data());
    }
    else {
        arr.push(parentNode && notTransTagName[parentNode.tagName()] ?
            html(node.data()) :
            node.data().replace(/[ ]{2}/g, " &nbsp;"));
    }
}
function insertElement(node, arr, formatter, current) {
    let attrhtml, attrs = node.attrs();
    if (attrs) {
        attrhtml = [];
    ...
    arr.push("<" +
        ...
        ">");
    const formatterByInsertLine = formatter &&
...
function insertComment(node, arr) {
    arr.push("<!--" + node.data() + "-->");
}
function getNodesById(root, id) {
...
function getNodesByTagName(root, tagName, arr) {
...
class MEVNode {
    ...
    _attrs;
    _children;
    static createElement(htmlOrTag) {
        if (/[<>]/.test(htmlOrTag)) {
    ...
    previousSibling() {
        ...
        if (parentNode) {
            ...
            for (let i = 0, ci; (ci = children[i]); i++) {
                if (ci === this && i !== 0) {
                    return children[i - 1];
                }
            }
    ...
    replaceChild(target, source) {
        target.remove();
        for (let i = 0, ci; (ci = this._children[i]); i++) {
            if (ci === source) {
    ...
    getNodeById(id) {
        let node;
        for (let i = 0, ci; (ci = this._children[i++]);) {
            if ((node = getNodesById(ci, id))) {
                return node;
            }
        }
    }
    getNodesByTagNames(tagNames) {
    ...
    attr(attrName, attrValue) {
        if ((typeof attrName === "string" && attrValue !== undefined) ||
            isObject(attrName)) {
    ...
    css(name, val) {
        ...
        const setStyleProperty = (name, val) => {
        ...
        if ((typeof name === "string" && val !== undefined) ||
            ...
            const reg = new RegExp("(^|;)\\s*" + name + ":([^;]+)", "i"), match = cssText.match(reg);
            if (match && match[0]) {
                return match[2];
            }
        }
    }
    applyStyle(nativMEVNode) {
        if (!nativMEVNode)
            return;
        const style = this.attr("style");
        if (style) {
            nativMEVNode.setAttribute("style", notTransAttrs["style"] ?
...
//先去掉了，加上的原因忘了，这里先记录
const re_tag = new RegExp("<(?:(?:\\/([^>]+)>)|(?:!--([\\S|\\s]*?)-->)|(?:([^\\s\\/<>]+)\\s*((?:(?:\"[^\"]*\")|(?:'[^']*')|[^\"'<>])*)\\/?>))", "g"), re_attr = /([\w\-:.]+)(?:(?:\s*=\s*(?:(?:"([^"]*)")|(?:'([^']*)')|([^\s>]+)))|(?=\s|$))/g;
//ie下取得的html可能会有\n存在，要去掉，在处理replace(/[\t\r\n]*/g,'');代码高亮的\n不能去除
...
function clearWhitespace(htmlStr, removeBlank) {
...
function appendText(parentNode, data) {
...
function appendElement(parentNode, tagName, htmlAttr) {
    const needParentTag = needParentNode[tagName];
    if (needParentTag) {
        let tmpParent = parentNode, hasParent;
        while (tmpParent && tmpParent.type() !== "root") {
            const tagName = tmpParent.tagName();
            if (Array.isArray(needParentTag) ?
        ...
        if (!hasParent) {
            parentNode = appendElement(parentNode, Array.isArray(needParentTag) ? needParentTag[0] : needParentTag);
        }
    }
...
function appendComment(parentNode, data) {
    parentNode.appendChild(MEVNode.createComment(data));
}
function HTMLParser(htmlStr, removeBlank) {
    const root = new MEVNode({
        type: "root",
        children: [],
    });
    let match, currentIndex = 0, lastIndex = 0, currentParent = root;
        ...
        try {
            ...
            if (match[3]) {
                if (dtd.$cdata[currentParent.tagName()]) {
                    ...
                    currentParent = appendElement(currentParent, match[3].toLowerCase(), match[4]);
                }
            }
            else if (match[1]) {
...
export { MEVNode, HTMLParser as default, html, unhtml };
