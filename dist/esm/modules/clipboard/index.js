...
import StateToMarkdown from '../state/stateToMarkdown.js';
import StateToHtml from '../state/stateToHtml.js';
import { generateId } from '../../utils/utils.js';
import { URL_REG } from '../../utils/url.js';
import { normalizePastedHTML, checkCopyType } from '../../utils/paste.js';
import { MathMLToLaTeX } from 'mathml-to-latex';
import HtmlToMarkdown from '../state/htmlToMarkdown.js';
import MarkdownToState from '../state/markdownToState.js';
import StateToPlainText from '../state/stateToPlainText.js';
import { imageUtils } from '../content/utils/image.js';
import { INLINE_UPDATE_REG, CODE_BLOCK_REG, MATH_BLOCK_REG, TABLE_BLOCK_REG, HTML_BLOCK_REG, convertIfNeeded } from '../content/utils/convert.js';
import { copyBlocks, cutBlocks } from './utils.js';
...
class MEClipboard extends MEModule {
    ...
    async processFiles(files) {
        ...
        const items = await Promise.all(Array.from(files).map(async (file) => {
            ...
            if (!src)
                src = imageUtils.createObjectURL(file);
