...
const getTextContent = (node, blackList) => {
    ...
    if (node.nodeType === 3) {
        ...
        const imageContainer = node.querySelector(`.${CLASS_NAMES.ME_IMAGE_CONTAINER}`);
    ...
    for (i = 0; i < len; i++) {
        ...
        if (/^\n$/.test(textContent) && i !== len - 1
            ...
            if (child.classList &&
                ...
                const imageContainer = child.querySelector(`.${CLASS_NAMES.ME_IMAGE_CONTAINER}`);
                const hasImg = imageContainer.querySelector("img");
                if (!hasImg) {
                    return {
                        node: child,
...
export { compareParagraphsOrder, findEditableDOM, getNodeAndOffset, getOffsetOfParent, getTextContent, isEditableDOM };
