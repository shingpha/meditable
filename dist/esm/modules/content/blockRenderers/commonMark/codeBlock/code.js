...
import { adjustOffset } from '../../utils.js';
import { insertTab } from '../paragraph/tab.js';
import { highlight } from './utlis/highlight.js';
import { HTML_TAGS, VOID_HTML_TAGS } from '../../../../../utils/tags.js';
...
const parseSelector = (str = '') => {
    ...
    for (const tagName of HTML_TAGS) {
        if (str.startsWith(tagName) && (!str[tagName.length] || /#|\./.test(str[tagName.length]))) {
            tag = tagName;
            if (VOID_HTML_TAGS.indexOf(tagName) > -1)
                isVoid = true;
...
const spacesToNbspOutsideTags = (html) => html
    ...
    : segment
        ...
        .replace(/ /g, '&nbsp;'))
    .join('');
class MECodeRenderer extends MEBlockRenderer {
    static type = "code";
    static tagName = 'code';
...
export { MECodeRenderer as default };
