 ...
 */
import { validEmoji } from '../../../../utils/emoji.js';
import { inlineRules, beginRules, validateRules } from './rules.js';
import { isLengthEven, validateEmphasize, lowerPriority, correctUrl, parseSrcAndTitle, getAttributes, union } from './utils.js';
...
const tokenizerFac = (src, beginRules, inlineRules, pos = 0, top, labels, options) => {
    ...
    while (src.length) {
        ...
        for (const rule of chunks) {
            ...
            if (to && isLengthEven(to[3])) {
                ...
                if (rule === "inline_code" ||
                    ...
                    rule === "inline_math") {
                    tokens.push({
                        type: rule === 'emoji' && !validEmoji(to[2]) ? 'emoji_valid' : rule,
                        raw: to[0],
                        range,
                        marker,
                        parent: tokens,
        ...
        correctUrl(imageTo);
        let srcAndTitle;
        if (imageTo && isLengthEven(imageTo[3]) && isLengthEven(imageTo[5]) && (srcAndTitle = parseSrcAndTitle(imageTo[4]))) {
        ...
        correctUrl(linkTo);
        if (linkTo && isLengthEven(linkTo[3]) && isLengthEven(linkTo[5]) && (srcAndTitle = parseSrcAndTitle(linkTo[4]))) {
