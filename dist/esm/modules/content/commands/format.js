 ...
 */
import { generator } from '../inlineRenderers/tokenizer/index.js';
...
const addFormat = (type, text, { start, end }) => {
    switch (type) {
        case 'em':
        case 'del':
        case 'inline_code':
...
const getFormatsInRange = (start, end, tokens) => {
    if (!start || !end) {
    ...
    (function iterator(tks) {
        for (const token of tks) {
            if (checkTokenIsInlineFormat(token) &&
...
export { FORMAT_MARKER_MAP, FORMAT_TYPES, addFormat, checkTokenIsInlineFormat, clearFormat, format, getFormatsInRange, getOffset, removeformat };
