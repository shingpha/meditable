...
class StateToMarkdown {
    listType;
    isLooseParentList;
    isGitlabCompatibilityEnabled;
    listIndentation;
    listIndentationCount;
    constructor({ listIndentation, isGitlabCompatibilityEnabled, } = {
        listIndentation: 1,
    ...
    convertStatesToMarkdown(states, indent = "", listIndent = "") {
        ...
        for (const state of states) {
            if (state.type !== "order-list" &&
                state.type !== "bullet-list" &&
                state.type !== "task-list") {
                lastListBullet = "";
            }
            switch (state.type) {
                case "frontmatter":
                ...
                case "atx-heading6":
                    ...
                    break;
                case "setext-heading1":
                case "setext-heading2":
                    this.insertLineBreak(result, indent);
                ...
                case "task-list": {
                    ...
                    this.listType.push(utils.deepCopy(meta));
    ...
    serializeAtxHeading(state, indent) {
        ...
        const atxHeadingText = match ? `${match[1]} ${match[2].trim()}` : text;
    ...
    serializeCodeBlock(state, indent) {
        const result = [];
        const { type } = state.meta;
        const { text, meta } = type === "fenced" ? state.children[1] : state.children[0];
    ...
    serializeTable(state, indent) {
        const result = [];
        const thead = state.children[0] || { type: "table-thead", children: [] };
        const tbody = state.children[1] || { type: "table-tbody", children: [] };
        const row = thead.children.length + tbody.children.length;
        const column = thead.children[0].children.length;
        const tableData = [];
        for (const rowState of thead.children) {
            tableData.push(rowState.children.map((cell) => escapeText(cell.text.trim())));
        }
        for (const rowState of tbody.children) {
            tableData.push(rowState.children.map((cell) => escapeText(cell.text.trim())));
        }
        const columnWidth = thead.children[0].children.map((th) => ({
            width: 5,
            align: th.meta.align,
        }));
        let i;
    ...
    serializeListItem(state, indent) {
        ...
        const { children, type, meta } = state;
        let itemMarker;
        if (isUnorderedList) {
...
module.exports = StateToMarkdown;
