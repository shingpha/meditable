...
class MEBlock extends module$1 {
    parent = null;
    id;
    type;
    children = [];
    ...
    set actived(state) {
        this.nodes.el.classList.toggle(classNames.CLASS_NAMES.ME_BLOCK__ACTIVED, state);
    }
    get actived() {
        return this.nodes.el.classList.contains(classNames.CLASS_NAMES.ME_BLOCK__ACTIVED);
    }
    set focused(state) {
        this.nodes.el.classList.toggle(classNames.CLASS_NAMES.ME_BLOCK__FOCUSED, state);
    }
    get focused() {
        return this.nodes.el.classList.contains(classNames.CLASS_NAMES.ME_BLOCK__FOCUSED);
    }
    set selected(state) {
        this.nodes.el.classList.toggle(classNames.CLASS_NAMES.ME_BLOCK__SELECTED, state);
    }
    get selected() {
        return this.nodes.el.classList.contains(classNames.CLASS_NAMES.ME_BLOCK__SELECTED);
    }
    set dropTarget(state) {
        this.nodes.el.classList.toggle(classNames.CLASS_NAMES.ME_BLOCK__DROP_TARGET, state);
    }
    get dropTarget() {
        return this.nodes.el.classList.contains(classNames.CLASS_NAMES.ME_BLOCK__DROP_TARGET);
    }
    render({ data, parent, cursor }) {
        this.type = data.type;
        this.id = data.id;
        this.parent = parent || null;
        patch.blockPatch.call(this, data, cursor, MEBlock);
        this.nodes.el.classList.toggle(classNames.CLASS_NAMES.ME_BLOCK, true);
    ...
    findBlock(el) {
        return find.findBlockByElement(el);
    }
    insert(options = {}) {
        ...
        data = data || { type: "paragraph", id: utils.generateId() };
