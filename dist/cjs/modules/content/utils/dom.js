...
const isEditableDOM = (element) => {
    return (element &&
        element.nodeType === 1 &&
        element.contentEditable === 'true');
};
const findEditableDOM = (node) => {
    if (!node) {
        return null;
    }
    do {
...
const getTextContent = (node, blackList) => {
    ...
    if (node.nodeType === 3) {
        ...
        node.classList.contains(`${classNames.CLASS_NAMES.ME_INLINE_IMAGE}`)) {
        // handle inline image
        const raw = node.getAttribute("data-raw");
        const imageContainer = node.querySelector(`.${classNames.CLASS_NAMES.ME_IMAGE_CONTAINER}`);
...
const getOffsetOfParent = (node, parent) => {
    let offset = 0;
    let preSibling = node;
    ...
    do {
        ...
        if (preSibling) {
            offset += getTextContent(preSibling, [
                classNames.CLASS_NAMES.ME_INLINE_RENDER
            ]).length;
        }
    } while (preSibling);
...
const getNodeAndOffset = (node, offset) => {
    ...
    for (i = 0; i < len; i++) {
        ...
        if (/^\n$/.test(textContent) && i !== len - 1
            ...
            : count + textLength >= offset) {
            if (child.classList &&
                child.classList.contains(`${classNames.CLASS_NAMES.ME_INLINE_IMAGE}`)) {
                const imageContainer = child.querySelector(`.${classNames.CLASS_NAMES.ME_IMAGE_CONTAINER}`);
                const hasImg = imageContainer.querySelector("img");
                if (!hasImg) {
                    return {
                        node: child,
...
const compareParagraphsOrder = (paragraph1, paragraph2) => {
    return (paragraph1.compareDocumentPosition(paragraph2) &
        Node.DOCUMENT_POSITION_FOLLOWING);
};

exports.compareParagraphsOrder = compareParagraphsOrder;
exports.findEditableDOM = findEditableDOM;
exports.getNodeAndOffset = getNodeAndOffset;
exports.getOffsetOfParent = getOffsetOfParent;
exports.getTextContent = getTextContent;
exports.isEditableDOM = isEditableDOM;
