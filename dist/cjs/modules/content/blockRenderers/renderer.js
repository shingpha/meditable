...
var index = require('../inlineRenderers/tokenizer/index.js');
...
class MEBlockRenderer extends eventHandler {
    static renders = {};
    static type = "staff";
    ...
    tokenizer() {
        ...
        return index.tokenizer(text, { hasBeginRules, labels });
    }
    setCursor({ anchor, focus, scrollToView } = {}) {
        const cursor = {
    ...
    autoPair(event, text, start, end, isInInlineMath = false, isInInlineCode = false, type = "format") {
        ...
        if (this.text !== text) {
            if (start.offset === end.offset && event.type === "input") {
                ...
                else if (event.inputType.indexOf("delete") === -1 &&
            ...
            else if (this.text.length === oldStart.offset &&
    ...
    forceUpdate(event) {
        ...
        // const { cursor } = selection
        const textContent = dom.getTextContent(this.nodes.holder, [classNames.CLASS_NAMES.ME_INLINE_RENDER]);
        const { text, needRender } = this.autoPair(event, textContent, start, end);
        // console.log(textContent, text, start.offset)
        if (this.block.type !== 'table-td' && this.block.type !== 'table-th' && this.block.type !== 'language' && convert.convertIfNeeded.call(this, text)) {
            return;
        }
        const cursor = {
    ...
    enterHandler(event) {
        ...
        convert.convertIfNeeded.call(paragraphBlock.renderer, textOfNewNode);
        if (this.block.type !== 'code') {
            convert.convertIfNeeded.call(this, text);
        }
    }
    backspaceHandler(event) {
        ...
        if (needRender) {
            event.preventDefault();
            const text = index.generator(tokens);
            start.offset--;
    ...
    handleClickInlineRuleRender(event, inlineRuleRenderEle) {
        ...
        const node = inlineRuleRenderEle.closest(`.${classNames.CLASS_NAMES.ME_NODE}`);
        const paragraph = dom.findEditableDOM(node);
        const offset = dom.getOffsetOfParent(node, paragraph);
    ...
    clickHandler(event) {
        ...
        const { target } = event;
        if (target.closest(`.${classNames.CLASS_NAMES.ME_TOOL}, .${classNames.CLASS_NAMES.ME_TOOLBAR}`)) {
            return;
        }
        const inlineRuleRenderEle = target.closest(`.${classNames.CLASS_NAMES.ME_INLINE_RENDER}`) ||
            (target.classList.contains(classNames.CLASS_NAMES.ME_NODE) && target.querySelector(`.${classNames.CLASS_NAMES.ME_INLINE_RENDER}`));
    ...
    arrowHandler(event) {
        ...
        const { topOffset, bottomOffset } = utils$1.getCursorYOffset(this.nodes.el, rangeRect);
        // Just do nothing if the cursor is not collapsed or `shiftKey` pressed
        if (start.offset !== end.offset ||
            event.shiftKey) {
            return;
        }
        if ((event.key === domUtils.keys.ArrowUp && topOffset > 0) ||
        ...
        let offset = 0;
        if ((event.key === domUtils.keys.ArrowUp) ||
        ...
        else if ((event.key === domUtils.keys.ArrowDown) ||
            ...
            cursorBlock && (offset = utils$1.adjustOffset(0, cursorBlock, event));
        }
...
exports.BACK_HASH = BACK_HASH;
exports.BRACKET_HASH = BRACKET_HASH;
exports.default = MEBlockRenderer;
