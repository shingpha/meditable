...
var tab = require('../paragraph/tab.js');
var highlight = require('./utlis/highlight.js');
...
const parseSelector = (str = '') => {
    ...
    let isVoid = false;
    let cap;
    for (const tagName of tags.HTML_TAGS) {
        if (str.startsWith(tagName) && (!str[tagName.length] || /#|\./.test(str[tagName.length]))) {
            tag = tagName;
            if (tags.VOID_HTML_TAGS.indexOf(tagName) > -1)
                isVoid = true;
...
/** 仅处理标签外的文本：空格 → &nbsp;，换行（\r\n / \n / \r）→ <br/>，避免破坏属性 */
const spacesToNbspOutsideTags = (html) => html
    .split(/(<[^>]+>)/)
    ...
    : segment
        ...
        .replace(/ /g, '&nbsp;'))
    .join('');
class MECodeRenderer extends renderer.default {
    static type = "code";
    static tagName = 'code';
    static async staticRender({ data }) {
        const { value = '' } = highlight.highlight(data.meta.lang, data.text);
    ...
    updateContent() {
        if (!this.nodes.el) {
            ...
            this.nodes.holder = this.make('span', [classNames.CLASS_NAMES.ME_EDITABLE, classNames.CLASS_NAMES.ME_CODE], { contenteditable: 'true' });
            this.nodes.el.appendChild(this.nodes.holder);
            const copyNode = this.make('span', [classNames.CLASS_NAMES.ME_CODE_COPY, classNames.CLASS_NAMES.ME_TOOL], { contenteditable: 'false' });
    ...
    enterHandler(event) {
        ...
        if (event.shiftKey) {
            ...
            const offset = utils$1.adjustOffset(0, cursorBlock, event);
    ...
    copyHandler(event) {
        ...
        event.target.classList.toggle(classNames.CLASS_NAMES.ME_CODE_COPY__SUCCESS, true);
        setTimeout(() => {
            event.target.classList.toggle(classNames.CLASS_NAMES.ME_CODE_COPY__SUCCESS, false);
        }, 1000);
    }
}

module.exports = MECodeRenderer;
