...
const tokenizerFac = (src, beginRules, inlineRules, pos = 0, top, labels, options) => {
    ...
    if (beginRules && pos === 0) {
        ...
        if (def && utils.isLengthEven(def[3])) {
            const token = {
                type: "",
                parent: tokens,
                leftBracket: def[1],
                ...
                children: tokenizerFac(def[2] + (def[3] || ""), undefined, inlineRules, pos + def[1].length, false, labels, options),
            };
            tokens.push(token);
    ...
    while (src.length) {
        ...
        for (const rule of emRules) {
            ...
            if (to && utils.isLengthEven(to[3])) {
                const isValid = utils.validateEmphasize(src, to[0].length, to[1], pending, rules.validateRules);
                if (isValid) {
                    inChunk = true;
                    pushPending();
                    const range = {
                    ...
                    tokens.push({
                        ...
                        marker,
                        parent: tokens,
                        children: tokenizerFac(to[2] + to[3], undefined, inlineRules, pos + to[1].length, false, labels, options),
                        backlash: to[3],
                    });
                    src = src.substring(to[0].length);
                    pos = pos + to[0].length;
        ...
        for (const rule of chunks) {
            ...
            if (to && utils.isLengthEven(to[3])) {
                if (rule === "emoji" &&
                    !utils.lowerPriority(src, to[0].length, rules.validateRules))
                    break;
                inChunk = true;
                pushPending();
                const range = {
                ...
                if (rule === "inline_code" ||
                    ...
                    rule === "inline_math") {
                    tokens.push({
                        type: rule === 'emoji' && !emoji.validEmoji(to[2]) ? 'emoji_valid' : rule,
                        raw: to[0],
                        range,
                        marker,
                        parent: tokens,
        ...
        utils.correctUrl(imageTo);
        let srcAndTitle;
        if (imageTo && utils.isLengthEven(imageTo[3]) && utils.isLengthEven(imageTo[5]) && (srcAndTitle = utils.parseSrcAndTitle(imageTo[4]))) {
            const { src: imageSrc, title } = srcAndTitle;
            pushPending();
            tokens.push({
                type: "image",
                raw: imageTo[0],
        ...
        utils.correctUrl(linkTo);
        if (linkTo && utils.isLengthEven(linkTo[3]) && utils.isLengthEven(linkTo[5]) && (srcAndTitle = utils.parseSrcAndTitle(linkTo[4]))) {
            const { src: href, title } = srcAndTitle;
            const anchor = linkTo[2] + linkTo[3];
            pushPending();
            tokens.push({
                type: !!anchor ? "link" : "link_no_text",
                raw: linkTo[0],
                marker: linkTo[1],
                ...
                children: !!anchor ? tokenizerFac(anchor, undefined, inlineRules, pos + linkTo[1].length, false, labels, options) : undefined,
                backlash: {
                    first: linkTo[3],
                    second: linkTo[5],
                },
        ...
        if (rLinkTo &&
            ...
            utils.isLengthEven(rLinkTo[2]) &&
            utils.isLengthEven(rLinkTo[4])) {
            pushPending();
            tokens.push({
                type: "reference_link",
                raw: rLinkTo[0],
                ...
                children: tokenizerFac(rLinkTo[1] + rLinkTo[2], undefined, inlineRules, pos + 1, false, labels, options),
            });
        ...
        if (rImageTo &&
            ...
            utils.isLengthEven(rImageTo[2]) &&
            utils.isLengthEven(rImageTo[4])) {
            pushPending();
            tokens.push({
                ...
                parent: tokens,
                alt: (rImageTo[1] + rImageTo[2]),
                backlash: {
                    first: rImageTo[2],
                    second: rImageTo[4] || "",
                },
                label: (rImageTo[3] + (rImageTo[4] || "")) || (rImageTo[1] + rImageTo[2]),
                range: {
                    start: pos,
        ...
        if (htmlTo &&
            ...
            (attrs = utils.getAttributes(htmlTo[0]))) {
            const tag = htmlTo[3];
            const html = htmlTo[0];
            const len = htmlTo[0].length;
            const type = tag === 'img' ? 'html_img' : tag === 'br' ? 'html_br' : !htmlTo[5] ? 'html_valid_tag' : tag === 'ruby' ? 'html_ruby' : 'html_tag';
            pushPending();
            tokens.push({
                type,
                raw: html,
...
const tokenizer = (src, { highlights = [], hasBeginRules = true, labels = new Map(), options = { superSubScript: true, footnote: true }, } = {}) => {
    const tokens = tokenizerFac(src, hasBeginRules ? rules.beginRules : null, rules.inlineRules, 0, true, labels, options);
    const postTokenizer = (tokens) => {
        for (const token of tokens) {
            for (const light of highlights) {
                const highlight = utils.union(token.range, light);
                if (highlight) {
