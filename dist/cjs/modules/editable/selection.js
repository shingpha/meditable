...
class MESelection extends module$1 {
    _document;
    _root;
    _rootDocNode;
    _ranges;
    ...
    _cachedCursor;
    _bakRange;
    constructor(document, root, instance) {
        super(instance);
    ...
    get nativeSelection() {
        const doc = (this._rootDocNode || this._document);
    ...
    getRange() {
        const optimize = (range) => {
            let child = this._root.firstChild, collapsed = range.collapsed;
        ...
        const range$1 = new range(this._document, this._root, this);
        if (env.ie9below) ;
        else {
            const sel = this.nativeSelection;
            if (sel && sel.rangeCount) {
                const firstRange = sel.getRangeAt(0), lastRange = sel.getRangeAt(sel.rangeCount - 1);
                range$1
                    .setStart(firstRange.startContainer, firstRange.startOffset)
                    .setEnd(lastRange.endContainer, lastRange.endOffset);
                if (range$1.collapsed &&
                    range$1.startContainer &&
                    domUtils.isRoot(range$1.startContainer) &&
                    !range$1.startOffset) {
                    optimize(range$1);
                }
            }
                ...
                if (this._bakRange &&
                    ...
                    domUtils.inDoc(this._bakRange.startContainer, this._root)) {
    ...
    get rangeRect() {
        let sel = this.nativeSelection, range;
        let rect = {
            x: 0,
            y: 0,
            width: 0,
    ...
    getCursor() {
        ...
        const aOffset = anchorBlock ? dom.getOffsetOfParent(anchorNode, anchorBlock.nodes.holder) + anchorOffset : anchorOffset;
        const fOffset = focusBlock ? dom.getOffsetOfParent(focusNode, focusBlock.nodes.holder) + focusOffset : focusOffset;
        ...
        if (isCollapsed) {
            ...
            const order = dom.compareParagraphsOrder(aDom, fDom);
    ...
    setCursor(cursor) {
        ...
        const { node: anchorNode, offset: anchorOffset } = dom.getNodeAndOffset(anchorBlock.nodes.holder, anchor.offset);
        const { node: focusNode, offset: focusOffset } = dom.getNodeAndOffset(focusBlock.nodes.holder, focus.offset);
    ...
    setAnchor(node, offset = 0) {
        const range = this.getRangeAt(0);
        /** if found deepest node is native input */
        if (domUtils.isNativeInput(node)) {
            if (!domUtils.canSetCursor(node)) {
                return;
            }
            node.focus();
    ...
    select(startNode, startOffset, endNode, endOffset = 0) {
        const range = this.getRangeAt(0);
