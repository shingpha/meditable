...
function Lexer(opts) {
    ...
    if (this.options.pedantic) {
        this.rules = blockRules.pedantic;
    }
...
Lexer.prototype.token = function (src, top) {
    ...
    while (src) {
        ...
        if (cap) {
            ...
            if (lastToken && lastToken.type === "paragraph") {
                ...
                this.tokens.push({
                    ...
                    text: !this.options.pedantic ? utils.rtrim(cap, "\n") : cap,
                });
            }
            continue;
        }
        ...
        if (cap) {
            ...
            if (text.endsWith("#")) {
                const trimmed = utils.rtrim(text, "#");
                if (this.options.pedantic) {
                    text = trimmed.trim();
                }
        ...
        if (cap) {
            item = {
                type: "table",
                header: utils.splitCells(cap[1].replace(/^ *| *\| *$/g, "")),
            ...
            if (item.header.length === item.align.length) {
                ...
                for (i = 0; i < item.cells.length; i++) {
                    item.cells[i] = utils.splitCells(item.cells[i], item.header.length);
                }
        ...
        if (cap) {
            ...
            if (item.header.length === item.align.length) {
                ...
                for (i = 0; i < item.cells.length; i++) {
                    item.cells[i] = utils.splitCells(item.cells[i].replace(/^ *\| *| *\| *$/g, ""), item.header.length);
                }
                this.tokens.push(item);
                continue;
            }
