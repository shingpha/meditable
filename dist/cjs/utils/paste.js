...
function preprocessWordPaste(html) {
    ...
    for (const el of all) {
        ...
        if (!fw && !fi && !fu && !fs)
            continue;
        const wrappers = [];
        if (fw && !el.closest("strong,b"))
            wrappers.push("strong");
        if (fi && !el.closest("em,i"))
            wrappers.push("em");
        if (fu && !el.closest("u"))
            wrappers.push("u");
        if (fs && !el.closest("s,del,strike"))
            wrappers.push("s");
        if (!wrappers.length)
            continue;
        let outer = null;
    ...
    const topParas = Array.from(body.children).filter((c) => c.tagName === "P");
    for (const p of topParas) {
        ...
        if (!level) {
            m = /mso-outline-level\s*:\s*(\d)/i.exec(style);
            if (m)
                level = parseInt(m[1], 10);
        }
        if (!level) {
            m = /(?:heading|标题)\s*(\d)/i.exec(styleName);
            if (m)
                level = parseInt(m[1], 10);
        }
        if (level >= 1 && level <= 6) {
    ...
    while (li < kids.length) {
        ...
        for (const q of group) {
            ...
            Array.from(q.querySelectorAll("span")).forEach((sp) => {
                if (/mso-list\s*:\s*Ignore/i.test((sp.getAttribute("style") || "")))
                    sp.remove();
            });
            while (q.firstChild)
                item.appendChild(q.firstChild);
    ...
    const maths = Array.from(body.querySelectorAll("math"));
    for (const m of maths) {
        let latex = "";
        try {
            latex = mathmlToLatex.MathMLToLaTeX.convert(m.outerHTML);
        }
        catch (e) {
            latex = "";
        }
        if (!latex) {
            m.replaceWith(doc.createTextNode(" "));
            continue;
        }
        const isBlock = m.hasAttribute("display") ||
            /display\s*:\s*(block|true)/i.test(m.getAttribute("style") || "") ||
            !!m.querySelector("mstyle[displaystyle=\"true\"]");
        m.replaceWith(doc.createTextNode(isBlock ? "$$" + latex + "$$" : "$" + latex + "$"));
    }
    return body.innerHTML;
}
const normalizePastedHTML = async function (html) {
    ...
    const sanitizedHtml = dompurify.default(html, dompurify.PREVIEW_DOMPURIFY_CONFIG);
 ...
 */
const checkCopyType = function (html, text, pasteType) {
    const getCopyType = (text) => {
        const match = /^<([a-zA-Z\d-]+)(?=\s|>).*?>[\s\S]+?<\/([a-zA-Z\d-]+)>$/.exec(text.trim());
        if (match && match[1]) {
            const tag = match[1];
            return nodeTypes.PARAGRAPH_TYPES.find((type) => type === tag) ? "code" : "text";
        }
        return "text";
    };
    if (pasteType === "normal") {
...
exports.checkCopyType = checkCopyType;
