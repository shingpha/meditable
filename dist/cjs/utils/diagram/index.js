...
const loadRenderer = async (name) => {
    if (!rendererCache.has(name)) {
        ...
        switch (name) {
            case "sequence":
                m = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespaceDefaultOnly(require('./sequence/index.js')); });
                rendererCache.set(name, m.default);
                break;
            case "plantuml":
                m = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespaceDefaultOnly(require('./plantuml/index.js')); });
                rendererCache.set(name, m.default);
                break;
            case "flowchart":
...
const renderDiagram = async ({ type, code, target, theme }) => {
    ...
    if (type === 'flowchart' || type === 'sequence') {
        ...
        target.innerHTML = dompurify.default(code, dompurify.PREVIEW_DOMPURIFY_CONFIG);
