...
function getDomNode(node, start, ltr, startFromChild, fn, guard) {
    let tmpNode = startFromChild && node[start], parent = null;
...
const attrFix = env.ie && env.version < 9 ? {
    tabindex: "tabIndex",
    readonly: "readOnly",
    for: "htmlFor",
    class: "className",
...
const domUtils = {
    ...
    getPosition(nodeA, nodeB) {
        ...
        let node, parentsA = [nodeA], parentsB = [nodeB];
        node = nodeA;
    ...
    findParents(node, includeSelf, filterFn, closerFirst) {
        let parents = includeSelf && node && ((filterFn && filterFn(node)) || !filterFn) ? [node] : [];
    ...
    remove(node, keepChildren) {
        let parent = node.parentNode, child;
        if (parent) {
    ...
    on(element, type, handler) {
        let types = Array.isArray(type) ? type : type.trim().split(/\s+/), k = types.length;
        if (k) {
            while (k--) {
                type = types[k];
    ...
    isSameElement(nodeA, nodeB) {
        ...
        let thisAttrs = nodeA.attributes, otherAttrs = nodeB.attributes;
        ...
        if (env.ie) {
            for (let i = 0; (attrB = otherAttrs[i++]);) {
                if (attrB.specified) {
                    bl++;
                }
            }
    ...
    isSameStyle(nodeA, nodeB) {
        let styleA = nodeA.style.cssText
            .replace(/( ?; ?)/g, ";")
            .replace(/( ?: ?)/g, ":");
        let styleB = nodeB.style.cssText
            .replace(/( ?; ?)/g, ";")
            .replace(/( ?: ?)/g, ":");
        if (env.opera) {
            styleA = nodeA.style;
    ...
    isRoot(node) {
        ...
        return (parentNode &&
            node.nodeType === 1 &&
            (node.dataset.root || parentNode.nodeType === 9 || parentNode.nodeType === 11 /*|| (parentNode as HTMLElement).contentEditable === 'true'*/));
    },
    breakParent(node, parent) {
        let tmpNode, parentClone = node, clone = node, leftNodes, rightNodes;
        do {
                ...
                if (attrs) {
                    ...
                    if (style) {
                        ...
                        for (let j = 0, s; (s = style[j++]);) {
                            ci.style[domUtils.cssStyleToDomStyle(s.split(":")[0])] = s.split(":")[1];
                        }
                    }
                }
    ...
    getElementsByTagName(node, name, filter) {
        ...
        const names = name.trim().replace(/[ ]{2,}/g, " ")
            .split(" ");
        let arr = [];
        for (let n = 0, ni; (ni = names[n++]);) {
            let list = node.getElementsByTagName(ni);
            for (let i = 0, ci; (ci = list[i++]);) {
                if (!filterFn || filterFn(ci))
                    arr.push(ci);
            }
        }
        return arr;
    ...
    unSelectable: (env.ie && env.ie9below) || env.opera ?
        function (node) {
            //for ie9
    ...
    getComputedStyle(element, styleName) {
        ...
        try {
            if (env.webkit && styleName === "text-decoration") {
                value = domUtils.getWindow(element).getComputedStyle(element)["webkitTextDecorationsInEffect"];
            }
            else {
                value =
                    domUtils.getStyle(element, styleName) ||
                        (!!window.getComputedStyle ?
                            ...
                            (element.currentStyle || element.style)[domUtils.cssStyleToDomStyle(styleName)]);
            }
        }
        catch (e) {
            return "";
    ...
    removeClasses: function (elm, classNames) {
        classNames = Array.isArray(classNames) ?
            ...
            classNames
                ...
                .split(" ");
        let cls, i, ci;
        for (i = 0, cls = elm.className; (ci = classNames[i++]);) {
    ...
    addClass: function (elm, classNames) {
        ...
        const names = classNames
            ...
            .split(" ");
        let cls, i, ci;
        for (i = 0, cls = elm.className; (ci = names[i++]);) {
    ...
    hasClass(element, className) {
        ...
        const names = className.trim()
            ...
            .split(" ");
        let i, ci, cls;
        for (i = 0, cls = element.className; (ci = names[i++]);) {
    ...
    cssStyleToDomStyle: (function () {
        const test = document.createElement("div").style, cache = {
    ...
    getCSSText(element) {
        let cssText = element.style.cssText, css;
        if (cssText) {
            css = {};
            let styles = cssText.split(";");
            for (let i = 0, ci; (ci = styles[i]); i++) {
                ci = ci.trim();
    ...
    scrollToView(node, win, offsetTop) {
        function getViewPaneSize() {
            let doc = win.document, mode = doc.compatMode === "CSS1Compat";
            return {
                width: (mode ? doc.documentElement.clientWidth : doc.body.clientWidth) ||
                    0,
                height: (mode ? doc.documentElement.clientHeight : doc.body.clientHeight) ||
                    0,
            };
        }
    ...
    isStartInblock: function (range) {
        let tmpRange = range.cloneRange(), flag = 0, start = tmpRange.startContainer, tmp;
        ...
        while (!tmpRange.startOffset) {
            ...
            let pre = tmpRange.startContainer && tmpRange.startContainer.previousSibling, tmpNode;
            if (!pre) {
    ...
    isEmptyBlock: function (node, reg) {
        ...
        if (node[env.ie ? "innerText" : "textContent"].replace(reg, "").length > 0) {
            return false;
        }
    ...
    moveChild: function (src, dst, first) {
    ...
    filterNodeList: function (nodelist, filter, forAll) {
        ...
        if (typeof filter !== 'function') {
            ...
            filterFn = function (n) {
                if (n && n.tagName) {
                    return (tagNames.findIndex((t) => n.tagName.toLowerCase() === t) !== -1);
                }
                else {
                    return false;
                }
    ...
    make: function (tagName, classNames = null, attributes = {}, dataset = {}) {
